...
for cache in partial_labels/INRIA-Websearch_*_partial_labels_len_[2345].mat; do
    [[ -f "$cache" ]] || continue
...
# Existing dataset settings, with EMA explicitly fixed to 0.95 by request.
for partial_length in 2 3 4 5; do
    run_dir="results/consensus/INRIA-Websearch_L${partial_length}_seed${seed}_$(date +%Y%m%d_%H%M%S_%N)"
    mkdir -- "$run_dir"
    "${PYTHON:-python}" -B -u train.py \
        --dataset INRIA-Websearch --partial_length "$partial_length" --seed "$seed" \
